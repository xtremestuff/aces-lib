...
const double CINEMA_WHITE = 48.0;
const double CINEMA_BLACK = pow10f(log10f(0.02)); // CINEMA_WHITE / 2400. 
...
const double DIM_SURROUND_GAMMA = 0.9811;

// Saturation compensation factor
const double ODT_SAT_FACTOR = 0.93;
const array<array <double, 3>, 3> ODT_SAT_MAT = calc_sat_adjust_matrix( ODT_SAT_FACTOR, AP1_RGB2Y);




const array<array <double, 3>, 3> D60_2_D65_CAT = calculate_cat_matrix( AP0.white, REC709_PRI.white);




double Y_2_linCV( double Y, double Ymax, double Ymin) 
{
  return (Y - Ymin) / (Ymax - Ymin);
}

double linCV_2_Y( double linCV, double Ymax, double Ymin) 
{
  return linCV * (Ymax - Ymin) + Ymin;
}

array <double, 3> Y_2_linCV_f3(const array <double, 3> &Y, double Ymax, double Ymin)
{
    array <double, 3> linCV;
...
array <double, 3> linCV_2_Y_f3(const array <double, 3> &linCV, double Ymax, double Ymin)
{
    array <double, 3> Y;
...
array <double, 3> darkSurround_to_dimSurround(const array <double, 3> &linearCV)
{
...
array <double, 3> dimSurround_to_darkSurround(const array <double, 3> &linearCV)
{
  array <double, 3> XYZ = mult_f3_f44( linearCV, AP1_2_XYZ_MAT);
...
double roll_white_fwd( 
    double in,      // color value to adjust (white scaled to around 1.0)
    double new_wht, // white adjustment (e.g. 0.9 for 10% darkening)
    double width    // adjusted width (e.g. 0.25 for top quarter of the tone scale)
  )
{
    const double x0 = -1.0;
  ...
  )
{
    ...
    if ( -in < y0)
        ...
        c = c + in;
        const double discrim = sqrtf( b * b - 4.0 * a * c);
        const double t = ( 2.0 * c) / ( -discrim - b);
        out = -(( t * ( x1 - x0)) + x0);
    }
    return out;
}
